...
**WARNING >>>** Experiencing disappearing nodes? Try using `ghcr.io/haveagitgat/tdarr:2.23.01` instead of `ghcr.io/haveagitgat/tdarr:latest` (I have to). Since Tdarr transitioned to a new version, I've encountered issues where nodes vanish and database statistics become sluggish—even though their changelog promises performance improvements. This specific version provides reliable operation without the disappearing nodes problem.
